...
const StatusBadge = ({ estado }: { estado: 'activo' | 'inactivo' | 'mantenimiento' }) => {
    // 🔑 CORRECCIÓN CLAVE: Definir el tipo para el objeto de configuración
    type StatusConfig = {
      [key in 'activo' | 'inactivo' | 'mantenimiento']: {
        text: string;
        color: string;
        icon: JSX.Element;
      };
...
        const operarioId = usuarioLogueado.id_persona;
        
        const res = await axios.get(`http://localhost:4000/api/invernadero/por-operario/${operarioId}`);
