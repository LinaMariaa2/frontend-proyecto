...
import { useUser } from '@/app/context/UserContext'; // ⬅️ Importa tu contexto
...
export default function InvernaderosOperarioPage() {
  ...
  useEffect(() => {
    const obtenerInvernaderos = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:4000/api/invernadero/operario/${user.id_persona}`);
